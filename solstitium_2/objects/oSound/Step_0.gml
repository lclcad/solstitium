...
if(current_room != last_room)
{
	...
	{//RESTO
		if(audio_is_playing(soundMenu)){
			audio_sound_gain(soundMenu, 0, 500);
		}
		if(audio_is_playing(sound_boss)) audio_sound_gain(sound_boss, 0, 500);
		if(audio_is_playing(sound_lua)) audio_sound_gain(sound_lua, 0, 500);
		if(audio_is_playing(sound_boss_intro)) audio_sound_gain(sound_boss_intro, 0, 500);
...
{
	if(audio_sound_get_gain(sound_boss) == 0) audio_stop_sound(sound_boss);
	if(audio_sound_get_gain(sound_boss_intro) == 0) audio_stop_sound(sound_boss_intro);
	if(audio_sound_get_gain(sound_lua) == 0) audio_stop_sound(sound_lua);
	if(audio_sound_get_gain(soundMenu) == 0) audio_stop_sound(soundMenu);
	if(audio_sound_get_gain(soundTrack1) == 0) audio_stop_sound(soundTrack1);
}
